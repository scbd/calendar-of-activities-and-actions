# Prompt: Generate & Enforce Phase 2 Jira Issues

Use this prompt with an MCP-enabled assistant that can call a Jira MCP server (create & update issues).

Objective: For every unchecked Phase 2 checkbox task in `.github/requirements/roadmap.md`, ensure a fully compliant Jira issue exists or is updated to compliance. If branch `phase-2` does not exist, create it from the repository default branch. For each task, create its own working branch `p2-task-${taskNumber}` off `phase-2`. Keep commits as logical chunks and open a PR targeting `phase-2`.

Issues must be linked to the epic `DEV-739`. Each issue will have a due date of today + `1` days. Each summary MUST start with `CAA-P2-T${taskNumber} -` (e.g., `CAA-P2-T0 -` for the first task in the phase). Include the label `PHASE-2` and any other relevant labels from the set: `FOUNDATION`, `NUXT`, `TESTING`, `DOCS`, `CONFIG`, `UI`.
...
- Phase Label: `PHASE-2` (e.g., `PHASE-2`)
- Summary Prefix: `CAA-P2 -` (e.g., `CAA-P2 -`)
- Due Date: Today + `1` day(s)
- Allowed Issue Types: `Task` (default); allow `Improvement`/`New Feature` if justified

## Roadmap Source

Parse only Phase 2 list items from `.github/requirements/roadmap.md` under the heading exactly matching:
...
2. JQL search for existing issues with `summary ~ "CAA-P2 -"` containing the roadmap-derived unique token (slug of title). Prefer reuse over duplicate creation.
3. If missing, create the issue with mandatory fields populated (labels including `PHASE-2`, due date, summary prefix, description, and default type `Task`).
...
5. Create or update Git branches: ensure base branch `phase-2` exists; for each issue, create/update `p2-task-${taskNumber}` from base. Push and open PR if changes are made.
...
Always include `PHASE-2`. Add additional semantic labels from: `FOUNDATION`, `NUXT`, `TESTING`, `DOCS`, `CONFIG`, `UI` as appropriate (limit to ≤4 labels total). Derive additional labels automatically when helpful (e.g., presence of "Bootstrap" → `UI`, "Vitest" → `TESTING`).
...
- `PHASE_NAME` (required): the phase heading suffix exactly as in the roadmap, e.g., `Markdown Parsing`.
- `ROADMAP_PATH` (default: `.github/requirements/roadmap.md`).
- `JIRA_PROJECT_KEY` (default: `DEV`).
- `PARENT_EPIC_KEY` (e.g., `DEV-739`).
- `PHASE_LABEL` (default: `PHASE-2`).
- `SUMMARY_PREFIX` (default: `CAA-P2 -`).
