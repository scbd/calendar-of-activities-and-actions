# Prompt: Generate & Enforce Phase 8 Jira Issues

Use this prompt with an MCP-enabled assistant that can call a Jira MCP server (create & update issues).

Objective: For every unchecked Phase 8 checkbox task in `.github/requirements/roadmap.md`, ensure a fully compliant Jira issue exists or is updated to compliance. If branch `phase-8` does not exist, create it from the repository default branch. For each task, create its own working branch `p8-task-${taskNumber}` off `phase-8`. Keep commits as logical chunks and open a PR targeting `phase-8`.

Issues must be linked to the epic `DEV-739`. Each issue will have a due date of today + `2` days. Each summary MUST start with `CAA-P8-T${taskNumber} -` (e.g., `CAA-P8-T0 -` for the first task in the phase). Include the label `PHASE-8` and any other relevant labels from the set: `FOUNDATION`, `NUXT`, `TESTING`, `DOCS`, `CONFIG`, `UI`.
...
- Phase Label: `PHASE-8` (e.g., `PHASE-8`)
- Summary Prefix: `CAA-P8 -` (e.g., `CAA-P8 -`)
- Due Date: Today + `2` day(s)
- Allowed Issue Types: `Task` (default); allow `Improvement`/`New Feature` if justified

## Roadmap Source

Parse only Phase 8 list items from `.github/requirements/roadmap.md` under the heading exactly matching:
...
2. JQL search for existing issues with `summary ~ "CAA-P8 -"` containing the roadmap-derived unique token (slug of title). Prefer reuse over duplicate creation.
3. If missing, create the issue with mandatory fields populated (labels including `PHASE-8`, due date, summary prefix, description, and default type `Task`).
...
5. Create or update Git branches: ensure base branch `phase-8` exists; for each issue, create/update `p8-task-${taskNumber}` from base. Push and open PR if changes are made.
...
Always include `PHASE-8`. Add additional semantic labels from: `FOUNDATION`, `NUXT`, `TESTING`, `DOCS`, `CONFIG`, `UI` as appropriate (limit to ≤4 labels total). Derive additional labels automatically when helpful (e.g., presence of "Vitest" → `TESTING`, "Component" → `UI`, "CI" → `CONFIG`).
...
## Create a Phase 8 Start Report

- In `.github/prompts`, create file: `phase-8-start-report.md`.
...
- `PHASE_NUMBER` (required): `8`.
- `PHASE_NAME` (required): `Testing (E2E Deferred)`.
- `ROADMAP_PATH` (default: `.github/requirements/roadmap.md`).
- `JIRA_PROJECT_KEY` (default: `DEV`).
- `PARENT_EPIC_KEY` (e.g., `DEV-739`).
- `PHASE_LABEL` (default: `PHASE-8`).
- `SUMMARY_PREFIX` (default: `CAA-P8 -`).
...
- Focus on testing infrastructure and quality assurance while deferring E2E implementation to Phase 10 as specified.

## Phase 8 Specific Context

Phase 8 focuses on testing infrastructure with the following key points:

- **Unit Testing Priority**: Comprehensive unit test coverage using Vitest
- **Component Testing**: Vue component mount tests using @nuxt/test-utils
- **E2E Planning Only**: E2E test implementation explicitly deferred to Phase 10
- **CI Integration**: Adding continuous integration for automated testing
- **Coverage Requirements**: >80% coverage for critical paths
- **Quality Gates**: Ensuring tests handle edge cases and maintain code quality

Tasks should emphasize testing best practices, robust error handling, and preparation for future E2E implementation while maintaining the project's commitment to quality and maintainability.