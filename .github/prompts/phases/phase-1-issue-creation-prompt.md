# Prompt: Generate & Enforce Phase 1 Jira Issues

Use this prompt with an MCP-enabled assistant that can call a Jira MCP server (create & update issues).

Objective: For every unchecked Phase 1 checkbox task in `.github/requirements/roadmap.md`, ensure a fully compliant Jira issue exists or is updated to compliance. If branch `phase-1` does not exist, create it from the repository default branch. For each task, create its own working branch `p1-task-${taskNumber}` off `phase-1`. Keep commits as logical chunks and open a PR targeting `phase-1`.

Issues must be linked to the epic `DEV-739`. Each issue will have a due date of today + `1` days. Each summary MUST start with `CAA-P1-T${taskNumber} -` (e.g., `CAA-P1-T0 -` for the first task in the phase). Include the label `PHASE-1` and any other relevant labels from the set: `FOUNDATION`, `NUXT`, `TESTING`, `DOCS`, `CONFIG`, `UI`.
...
- Phase Label: `PHASE-1` (e.g., `PHASE-1`)
- Summary Prefix: `CAA-P1 -` (e.g., `CAA-P1 -`)
- Due Date: Today + `1` day(s)
- Allowed Issue Types: `Task` (default); allow `Improvement`/`New Feature` if justified

## Roadmap Source

Parse only Phase 1 list items from `.github/requirements/roadmap.md` under the heading exactly matching:
...
2. JQL search for existing issues with `summary ~ "CAA-P1 -"` containing the roadmap-derived unique token (slug of title). Prefer reuse over duplicate creation.
3. If missing, create the issue with mandatory fields populated (labels including `PHASE-1`, due date, summary prefix, description, and default type `Task`).
...
5. Create or update Git branches: ensure base branch `phase-1` exists; for each issue, create/update `p1-task-${taskNumber}` from base. Push and open PR if changes are made.
...
Always include `PHASE-1`. Add additional semantic labels from: `FOUNDATION`, `NUXT`, `TESTING`, `DOCS`, `CONFIG`, `UI` as appropriate (limit to ≤4 labels total). Derive additional labels automatically when helpful (e.g., presence of "Bootstrap" → `UI`, "Vitest" → `TESTING`).
...
## Create a Phase 1 Start Report

- In `.github/prompts/phases`, create file: `phase-1-start-report.md`.
...
- `PHASE_NAME` (required): the phase heading suffix exactly as in the roadmap, e.g., `Data Fetch (Indexer Task)`.
- `ROADMAP_PATH` (default: `.github/requirements/roadmap.md`).
- `JIRA_PROJECT_KEY` (default: `DEV`).
- `PARENT_EPIC_KEY` (e.g., `DEV-739`).
- `PHASE_LABEL` (default: `PHASE-1`).
- `SUMMARY_PREFIX` (default: `CAA-P1 -`).
