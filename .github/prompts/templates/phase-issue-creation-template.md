# Prompt: Generate & Enforce Phase {{PHASE_NUMBER}} Jira Issues

Use this prompt with an MCP-enabled assistant that can call a Jira MCP server (create & update issues).

Objective: For every unchecked Phase {{PHASE_NUMBER}} checkbox task in `{{ROADMAP_PATH}}`, ensure a fully compliant Jira issue exists or is updated to compliance. If branch `phase-{{PHASE_NUMBER}}` does not exist, create it from the repository default branch. For each task, create its own working branch `p{{PHASE_NUMBER}}-task-${taskNumber}` off `phase-{{PHASE_NUMBER}}`. Keep commits as logical chunks and open a PR targeting `phase-{{PHASE_NUMBER}}`.

Issues must be linked to the epic `{{PARENT_EPIC_KEY}}`. Each issue will have a due date of today + `{{DUE_DATE_OFFSET_DAYS}}` days. Each summary MUST start with `{{SUMMARY_PREFIX}}T${taskNumber} -` (e.g., `CAA-P{{PHASE_NUMBER}}-T0 -` for the first task in the phase). Include the label `{{PHASE_LABEL}}` and any other relevant labels from the set: `FOUNDATION`, `NUXT`, `TESTING`, `DOCS`, `CONFIG`, `UI`.
...
- Project Key: `{{JIRA_PROJECT_KEY}}` (default: `DEV`)
- Parent Epic: `{{PARENT_EPIC_KEY}}` (e.g., `DEV-739`)
- Phase Label: `{{PHASE_LABEL}}` (e.g., `PHASE-{{PHASE_NUMBER}}`)
- Summary Prefix: `{{SUMMARY_PREFIX}}` (e.g., `CAA-P{{PHASE_NUMBER}} -`)
- Due Date: Today + `{{DUE_DATE_OFFSET_DAYS}}` day(s)
- Allowed Issue Types: `Task` (default); allow `Improvement`/`New Feature` if justified

## Roadmap Source

Parse only Phase {{PHASE_NUMBER}} list items from `{{ROADMAP_PATH}}` under the heading exactly matching:
...
Note: Due to an MCP limitation preventing some custom fields from being set during creation, create the issue first and then immediately update it to set custom fields like Epic Link and Story Points. See enforcement template at `{{ENFORCEMENT_TEMPLATE_PATH}}` for examples and schema details.

1. Preflight: discover custom fields (Epic Link, Story Points) unless provided via env.
2. JQL search for existing issues with `summary ~ "{{SUMMARY_PREFIX}}"` containing the roadmap-derived unique token (slug of title). Prefer reuse over duplicate creation.
3. If missing, create the issue with mandatory fields populated (labels including `{{PHASE_LABEL}}`, due date, summary prefix, description, and default type `Task`).
4. Immediately PATCH to set Epic link (`{{PARENT_EPIC_KEY}}`), Story Points, timetracking original estimate, assignee (`JIRA_ASSIGNEE_ACCOUNT_ID` if present), custom fields, and any missing metadata.
5. Create or update Git branches: ensure base branch `phase-{{PHASE_NUMBER}}` exists; for each issue, create/update `p{{PHASE_NUMBER}}-task-${taskNumber}` from base. Push and open PR if changes are made.
...
Always include `{{PHASE_LABEL}}`. Add additional semantic labels from: `FOUNDATION`, `NUXT`, `TESTING`, `DOCS`, `CONFIG`, `UI` as appropriate (limit to ≤4 labels total). Derive additional labels automatically when helpful (e.g., presence of "Bootstrap" → `UI`, "Vitest" → `TESTING`).

## Required Field Compliance

Fail the run if any newly created or updated issue ends without:

- Epic Link (to `{{PARENT_EPIC_KEY}}`)
- Story Points value
- Plan start date and time and end date and time
- Original Estimate (timetracking.originalEstimate)
- Assignee
- `{{PHASE_LABEL}}` label
- Summary starting with `{{SUMMARY_PREFIX}}`
...
- If any are missing, immediately call the Jira REST API to set only the missing fields via PATCH, using the dynamically discovered custom field IDs (see `{{ENFORCEMENT_TEMPLATE_PATH}}`).
...
## Create a Phase {{PHASE_NUMBER}} Start Report

- In `.github/prompts/phases`, create file: `phase-{{PHASE_NUMBER}}-start-report.md`.
...
- `PHASE_NAME` (required): the phase heading suffix exactly as in the roadmap, e.g., `Foundations & Setup`.
- `ROADMAP_PATH` (default: `.github/requirements/roadmap.md`).
- `JIRA_PROJECT_KEY` (default: `DEV`).
- `PARENT_EPIC_KEY` (e.g., `DEV-739`).
- `PHASE_LABEL` (default: `PHASE-{{PHASE_NUMBER}}`).
- `SUMMARY_PREFIX` (default: `CAA-P{{PHASE_NUMBER}} -`).
